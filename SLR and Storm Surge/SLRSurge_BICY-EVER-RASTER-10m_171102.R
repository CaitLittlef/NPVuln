...
####################################################################
### BICY - CREATE BASELINE & FUTURE PARKS; COMPUTE & WRITE AREAS ###
####################################################################

start <- Sys.time()

# Load park shapefile
(park <- parks[which(parks$UNIT_CODE == "BICY"),]) # in lat/long
...
print(paste0("Done with BICY SLR", parks$UNIT_CODE[i], "."))
print(Sys.time() - start)
...
csvFileName <- paste0("BICY_SLR_resultz_",currentDate,".csv")
...
surge1 <- st_read(surge.db, "BICY10m_low_85_2050_C5H") # 
surge2 <- st_read(surge.db, "BICY10m_slr_85_2050_C5H")
surge3 <- st_read(surge.db, "BICY5m_low_85_2050_C5H")
surge4 <- st_read(surge.db, "BICY5m_slr_85_2050_C5H")
...
print(paste0("Done with BICY Surge", parks$UNIT_CODE[i], "."))
print(Sys.time() - start)
...
# Write results to csv
currentDate <- Sys.Date()
csvFileName <- paste0("BICY_resultz_surge_",currentDate,".csv")
